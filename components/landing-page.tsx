      ...
      <main className="relative z-10 mx-auto max-w-7xl px-6 pb-32 pt-12">
        <nav className="mb-16 flex items-center justify-between md:mb-24">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="rounded-xl font-bold hover:bg-secondary"
            >
              Moderator login
            </Button>
          </div>
        ...
        <div className="grid items-center gap-16 lg:grid-cols-12">
          <div className="lg:col-span-7">
            <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-indigo-500/20 bg-indigo-500/10 px-4 py-1.5 text-xs font-black uppercase tracking-widest text-indigo-500">
            ...
            <div className="mb-16 flex flex-wrap gap-4">
              <Button
                size="lg"
                onClick={() => router.push("/moderator")}
                className="h-14 rounded-2xl bg-indigo-600 px-8 text-lg font-black text-white shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 sm:h-16"
              >
              ...
              <Button
                variant="outline"
                size="lg"
                className="h-14 rounded-2xl border-border px-8 text-lg font-black sm:h-16"
                onClick={() =>
                  document
              ...
              <form onSubmit={handleJoinQuiz} className="relative space-y-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-14 border-border bg-secondary/30 text-lg font-bold placeholder:text-muted-foreground/50 sm:h-16"
                  />
                </div>
                <Button
                  type="submit"
                  disabled={!teamId.trim()}
                  className="h-16 w-full rounded-[2rem] text-xl font-black shadow-2xl shadow-indigo-600/30 group/btn sm:h-20 sm:text-2xl"
                >
                  Enter arena
...
function FeatureCard({
  ...
  const colors: Record<string, string> = {
    ...
    pink: "border-pink-500/20 bg-pink-500/10 text-pink-500",
  };
  ...
  return (
    <div className="glass-card rounded-[2rem] p-8 transition-transform duration-300 hover:-translate-y-2 md:p-10">
      ...
      <p className="font-medium leading-relaxed text-muted-foreground">{desc}</p>
    </div>
  );
}
